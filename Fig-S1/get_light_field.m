function [I_ic,I_mp,I_net] = get_light_field(pond_X,pond_Y,z,edge_mp)

SW = 500;

light = SW*comp_light_field(pond_X,pond_Y,z,edge_mp);
...
% Increased absorption in bare regions
meltpond(meltpond==1)=I_p*exp(-thick*k_p)*(1-alpha_w); % 0.4221; from before
% meltpond(meltpond==0)=exp(-thick*k_i)*(1-alpha_i); % 0.12; from before
meltpond(meltpond==0)=I_b*exp(-thick*k_i)*(1-alpha_i); % 0.092; from before
