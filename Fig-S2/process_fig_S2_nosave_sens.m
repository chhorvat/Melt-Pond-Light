...
if ~isempty(stats)
    ...
    light = SW*comp_light_field_sens(pond_X,pond_Y,z,edge_mp,kappa_pert);
    ...
    I_ic_save{num_it} = I_ic;
    I_mp_save{num_it} = I_mp;
    I_net_save{num_it} = I_net;
    x_save{num_it} = pond_X;
    y_save{num_it} = pond_Y;
    z_save{num_it} = z;
    
    %    save(['pseudo-pond-stats-' num2str(nx) '.mat'],'*_save','num_it');
